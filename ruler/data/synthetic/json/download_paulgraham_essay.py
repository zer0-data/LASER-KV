...
urls_file = os.path.join(SCRIPT_DIR, 'PaulGrahamEssays_URLs.txt')
if not os.path.exists(urls_file):
    print("Downloading PaulGrahamEssays_URLs.txt from reference repo...")
    urllib.request.urlretrieve(
        "https://raw.githubusercontent.com/zer0-data/Statistical-Cross-Attention"
        "/main/ruler/data/synthetic/json/PaulGrahamEssays_URLs.txt",
        urls_file,
    )
...
print(f'Downloaded {len(files_repo)} essays from github.com/gkamradt/LLMTest_NeedleInAHaystack')
print(f'Downloaded {len(files_html)} essays from paulgraham.com')
...
out = os.path.join(SCRIPT_DIR, 'PaulGrahamEssays.json')
